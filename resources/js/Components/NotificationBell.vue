...
<template>
    <div ref="root" class="relative">
        <button type="button" class="relative flex h-9 w-9 items-center justify-center rounded-full text-gray-500 transition hover:bg-gray-100" aria-label="Мэдэгдэл" @click="toggle">
            ...
            <span v-if="data.unread" class="absolute -right-0.5 -top-0.5 flex h-4 min-w-[16px] items-center justify-center rounded-md bg-red-500 px-1 text-[10px] font-bold text-white">{{ data.unread > 9 ? '9+' : data.unread }}</span>
        </button>
