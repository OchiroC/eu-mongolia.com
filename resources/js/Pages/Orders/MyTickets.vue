...
<template>
    ...
    <PublicLayout>
        <h1 class="mb-6 text-2xl font-semibold">Миний тасалбарууд</h1>
            ...
            >
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500"><CalendarDays class="mr-1.5 inline-block h-4 w-4 align-[-3px]" />{{ formatDate(order.event?.starts_at) }}</p>
                        <p class="mt-1 text-xs text-gray-400">#{{ order.reference }}</p>
                    </div>
                    <span class="rounded-md bg-brand-50 px-3 py-1 text-sm font-medium text-brand-700">
