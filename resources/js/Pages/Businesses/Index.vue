...
<template>
    ...
    <PublicLayout>
        ...
        <div v-if="businesses.data.length" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <Link v-for="b in businesses.data" :key="b.id" :href="`/businesses/${b.slug}`" class="group flex items-center gap-3 rounded-2xl border border-gray-100 bg-white p-3 shadow-card transition duration-300 hover:-translate-y-1 hover:shadow-card-lg">
