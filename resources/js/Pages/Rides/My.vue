...
<template>
    ...
    <PublicLayout>
        <div class="mb-6 flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-gray-900">Миний аялал</h1>
            ...
            <div v-for="r in rides" :key="r.id" class="flex flex-col gap-3 rounded-2xl bg-white p-4 shadow-soft ring-1 ring-gray-100 sm:flex-row sm:items-center sm:justify-between">
                <div class="min-w-0">
                    <div class="flex items-center gap-2">
                        <span class="rounded-md px-2 py-0.5 text-xs font-medium" :class="statusClass[r.status]">{{ statusLabel[r.status] }}</span>
                    </div>
                    <Link :href="`/rides/${r.id}`" class="mt-1 block font-semibold text-gray-900 hover:text-brand-700">{{ r.from_city }} → {{ r.to_city }}</Link>
                    <p class="text-sm text-gray-500"><Clock class="mr-1 inline-block h-3.5 w-3.5 align-[-2px]" />{{ formatDateTime(r.depart_at) }} · {{ r.seats }} суудал<span v-if="r.price"> · {{ r.price }}</span></p>
                </div>
