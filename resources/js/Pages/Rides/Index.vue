...
<template>
    ...
    <PublicLayout>
        <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <Link :href="user ? '/rides/new' : '/login'" class="inline-flex w-fit items-center gap-1.5 rounded-full bg-brand-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition duration-200 hover:-translate-y-0.5 hover:bg-brand-700 hover:shadow-brand-glow active:translate-y-0">
        ...
        <div v-if="rides.data.length" class="space-y-3">
            <Link
                ...
                :key="r.id"
                :href="`/rides/${r.id}`"
                class="group flex items-center justify-between gap-4 rounded-2xl border border-gray-100 bg-white p-4 shadow-card transition duration-300 hover:-translate-y-1 hover:shadow-card-lg"
            >
                <div class="min-w-0">
